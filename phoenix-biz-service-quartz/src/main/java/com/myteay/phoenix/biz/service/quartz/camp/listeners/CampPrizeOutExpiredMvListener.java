 ...
 * 
 * @author danlley
 * @version $Id: CampPrizeOutExpiredMvListener.java, v 0.1 May 8, 2019 11:42:16 PM danlley Exp $
 */
public class CampPrizeOutExpiredMvListener extends EventListener<Object> {
    ...
    public Object handleEvent(MtEvent<?> event) {
        ...
        try {
            campShopPrizeOutModels = campShopPrizeOutRepository.selectCampShopPrizeOutListByStatus(CampPrizeOutStatusEnum.CAMP_PRIZE_OUT_EXPIRED);
        } catch (PxManageException e) {
            logger.warn("查询已过期的奖品流水信息模型失败 " + e.getMessage(), e);
        }

        if (CollectionUtils.isEmpty(campShopPrizeOutModels)) {
            if (logger.isInfoEnabled()) {
                logger.info("当前未找到已过期的奖品，本次定时扫描结束!");
            }
            return null;
        }
