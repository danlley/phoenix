...
) as a where sum_date = '2019-3-04'

-- 查询用户是否下单成功
select * from (
select *,date_format(gmt_created,'%Y-%c-%d') sum_date from px_goods_order_out) as a where sum_date = '2019-3-04' order by gmt_created desc

-- 查询冰淇淋的单日销量
select * from (
select *,date_format(gmt_created,'%Y-%c-%d') sum_date from px_goods_order_out) as a where sum_date = '2019-3-04' and goods_title like '%冰淇淋%' order by gmt_created desc
